def count_char(cadena, caracter):
    ...
    if not isinstance(cadena, str):  #Esta función se encarga de verificar que la cadena sea un string
        return (-100, None)  # En caso que la cadena no sea un string se retorna un código conforme al formato especificado anteriormente
    ...
    if not cadena.isalnum():  # Esta función se encarga de verificar que el parámetro cadena contenga solamente caracteres alfanumericos
        return (-200, None)  # En caso que la cadena no esté conformada puramente por caracteres alfanumericos se retorna un código conforme al formato especificado anteriormente
    ...
    if not isinstance(caracter, str) or len(caracter) != 1 or not caracter.isalnum():  # Esta línea cumple con dos condiciones distintas, la primera funci´pon verifica que el parámetro caracter sea un string, la segunda función se encarga de verificar que la longitud de caracted no sea mayor que 1
        return (-300, None)  # En caso de que no se cumplan las condiciones de tipo de dato y tamaño se devuelve un codigo de error conforme a lo especificado anteriormente
...
def multiplo_2(base, multiplo):
    ...
    if not (isinstance(base, int) and isinstance(multiplo, int)) or base <= 0 or multiplo <= 0:  # Se verifica que ambos parametros de entrada sean datos de tipó integer, además se verifica que estos sean enteros positivos.
        return (-400, None)  # En caso de que al menos uno de los parametros no sean enteros positivos se devuelve un código de error confome a lo especificado anteriormente
    
    # --- Verificación 2 ---
    if multiplo not in [1, 2, 4, 8, 16]: # Se verifica que multiplo sea 1, 2, 4, 8 o 16
        return (-500, None)  # En caso de que no lo sea, se devuelve un codigo de error conforme a lo establecido anteriormente
